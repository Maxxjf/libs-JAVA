...
public class ValidateUtil {
    ...
    /**
     * 验证是否至少100，不超4500的提现金额
     *
     * @param money 金额
     *
     * @return 返回true表示合法，false表示非法
     */
    public static boolean isFitCash(int money) {
        return money>=100&&money<=4500;
    }
     ...
     * @param mobile 手机号
     * */
    public static String setMobileToPassword(String mobile) {

        if (StringUtils.isNotEmptyString(mobile) && mobile.length() > 7 ) {
     ...
     * @param email 手机号
     * */
    public static String setEmailToPassword(String email) {

        if (StringUtils.isNotEmptyString(email) && email.length() > 7 ) {
            StringBuilder sb  =new StringBuilder();
